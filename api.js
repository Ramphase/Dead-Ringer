...
exports.setApp = function (app, client) {
  ...
  app.post("/editMessage", async (req, res, next) => {
    ...
    try {
      await Messages.findOneAndUpdate(
        ...
        { $set: { MessageName: newMessageName, Text: text } }
      );
    } catch (e) {
      error = e.toString();
    }
  ...
  app.post("/displayMessages", async (req, res, next) => {
    ...
    for (var i = 0; i < results.length; i++) {
      _ret.push({
        ...
        MessageName: results[i].MessageName,
        Text: results[i].Text,
      });
    }
  ...
  app.post("/addTrigger", async (req, res, next) => {
    ...
    const { userId, triggerName, messageName, contactId, time, jwtToken } =
      req.body;

    //Gets the message text from the recieved message name
    const msgResults = await Messages.find({
      UserId: userId,
      MessageName: messageName,
    });
    ...
    const results = await Triggers.find({
      UserId: userId,
      TriggerName: triggerName,
    });
  ...
  app.post("/editTriggerName", async (req, res, next) => {
    ...
    try {
      await Triggers.findOneAndUpdate(
        { UserId: userId, _id: triggerId },
        { $set: { TriggerName: newTriggerName } }
      );
    } catch (e) {
      error = e.toString();
    }
  ...
  app.post("/searchTriggers", async (req, res, next) => {
    ...
    const results = await Triggers.find({
      UserId: userId,
      TriggerName: { $regex: _search + ".*", $options: "i" },
    }); // changed 'r' to 'i'

    var _ret = [];
    for (var i = 0; i < results.length; i++) {
      _ret.push({
        TriggerName: results[i].TriggerName,
  ...
  app.post("/addContact", async (req, res, next) => {
    ...
    const { userId, firstName, lastName, email, phoneNumber, jwtToken } =
      req.body;
    const newContact = {
      UserId: userId,
      FirstName: firstName,
  ...
  app.post("/executeTrigger", async (req, res, next) => {
    ...
    const triggerResult = await Triggers.find({
      UserId: userId,
      TriggerName: triggerName,
    });
    ...
    try {
      await Triggers.find({ UserId: userId, TriggerName: triggerName })
        .deleteOne()
        .exec();
    } catch (e) {
      error = e.toString();
  ...
  app.post("/displayTriggers", async (req, res, next) => {
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      _ret.push({
        triggerId: results[i]._id,
        triggerName: results[i].TriggerName,
        message: results[i].Message,
        contactId: results[i].Contact,
        time: results[i].Time,
      });
    }
  ...
  setInterval(async () => {
    ...
    Triggers.updateMany({}, { $inc: { Time: -2 } }, function (err, docs) {
      if (err) {
        console.log(err);
      } else {
    ...
    if (triggerResults != null) {
      for (i = 0; i < triggerResults.length; i++) {
        ...
        var userIdResult = await Triggers.find({
        ...
        axios
          .post("https://dead-ringer.herokuapp.com/executeTrigger", {
            // change route
